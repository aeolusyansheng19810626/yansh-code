...
| 61-69 | yscode（缺口覆盖） | `AB-test/yscode/setup_yscode.py` + `2026-06-06_04/05/06` |
...
| 61 | yscode | 失败路径(不存在函数) | 给不存在的 `batch_read_files()` 加并发——应放弃 | ✅ | pass(正确放弃) |
| 62 | yscode | 矛盾需求(trick) | 不改 tools.py 却给 `search_in_files` 加 `max_results` | ✅ | **fail→豁免**(矛盾，yansh 绕过) |
| 63 | yscode | 删除死代码 | 删 `task_log.py` 注入的 `_legacy_serialize` 死函数 | ❌ | pass |
| 64 | yscode | 删除死参数 | 删 `main.py` 注入的 `--legacy-progress` | ❌ | pass |
| 65 | yscode | 正向全局重命名 | `directory_summary`→`summarize_directory`（4 文件引用） | ❌ | pass(R2) |
| 66 | yscode | 深调用链改签名 | `_dispatch_tool_calls` 加必传 `request_id`，4 处调用点 | ❌ | pass(R2) |
| 67 | yscode | 非py文件 | `requirements.txt` 补 `pydantic` | ❌ | pass |
| 68 | yscode | 非py文件 | `pyproject.toml` 补回删掉的 `console_shared` | ❌ | pass |
| 69 | yscode | 测试本身有 bug | `test_frontmatter` 断言 foo→bar 写错，应改测试非源码 | ❌ | pass |

> `*假通过`：task30/40/50 yansh 在 ws 根目录新建同名文件，测试测的是副本而非包内实现——表面 pass，实际未触及目标。修法：prompt 明确目标文件为 `tests/unit/test_*.py`。
> **task62 豁免**：需求自相矛盾（不改 tools.py 却要给其函数加参数），yansh 倾向找路子绕过而非喊停。已记为已知边界，不加矛盾检测 Pattern（误伤"合理包装扩展"风险高）。详见 `2026-06-06_06`。
...
| 重构(抽取/改型/重命名) | 8 | 5,8,13,15,55,58,65,66 |
| 只读分析(±文档) | 4 | 1,3,51,53 |
| 纯测试 | 3 | 30,40,50 |
| 性能优化 | 1 | 21 |
| 防御/校验 | 1 | 24 |
| 失败路径/矛盾(trick) | 3 | 2,61,62 |
| 删除/清理 | 2 | 63,64 |
| 非py文件编辑 | 2 | 67,68 |
| 测试本身有bug | 1 | 69 |
...
| 缺口 | 说明 | 覆盖 task | 状态 |
|------|------|-----------|------|
| **A. 失败路径鲁棒性** | 需求指向不存在的函数/矛盾需求——应 `success=False` 放弃而非幻觉硬写 | 61(不存在函数)、62(矛盾) | ✅ 61 通过；62 暴露边界(豁免) |
| **B. 删除/清理** | 删死代码/死参数 + 调用点 | 63、64 | ✅ 已覆盖 |
| **C. 正向全局重命名** | 跨多文件 rename 公开符号 | 65 | ✅ 已覆盖(R2 修 replace 谎报) |
| **D. 大改动/深调用链** | 改被多处引用的核心函数签名 | 66 | ✅ 已覆盖(R2 修轮次不足) |
| **E. 非 .py 文件** | 改 requirements/pyproject/README | 67、68 | ✅ 已覆盖 |
| **F. 测试本身有 bug** | 测试断言错，应改测试非源码 | 69 | ✅ 已覆盖 |
| **G. 并发安全修复** | 修竞态/加锁（非仅分析） | — | ⬜ 仍未覆盖（低优先级） |

> task61-69 最终 **8/9 通过 + task62 豁免**。本轮给 yansh 落地：费用熔断($50)/无进展熔断/放宽轮次(40)/Pattern12(失败放弃)/Pattern13(rename 验证)。详见 `2026-06-06_06`。
> 剩余缺口 G（并发安全修复）：需准备含竞态 bug 的目标代码 + 线程压测验证，工作量较大，暂缓。
